...
import cv2, warnings
import numpy as np
from GGUtils.img.img import RGBTuple, get_img_axis_size, img_corners
...
# 1.1. 회전(Rotation): 이미지의 중심을 기준으로 일정 각도로 회전. 일반적으로 -180에서 180 사이에서 선택
# > Rotate
# 1.2. 전단 변환(Shear Transformation): 이미지를 x 또는 y 축을 기준으로 평행하게 이동시킴.
# > Shear
# 1.3. 이동(Translation): 이미지를 x축과 y축 방향으로 일정 픽셀만큼 이동. 데이터의 위치 편향을 피할 수 있음.
# > Translate
# 1.4. 확대/축소(Scaling): 이미지를 확대하거나 축소
# > Scaling(무작위 비율로 크기 조정), FixedResize(고정된 크기로 이미지 크기 조정)
# 1.5. 반전(Flipping): 이미지를 수평 또는 수직 방향으로 반전
# > Flip
# 1.6. 크롭(Cropping): 이미지의 일부분을 잘라내어 사용
# > Crop
...
class Rotate:
    def __init__(
            self, p:float=1.0, max_degree:int=90,
        ...
        self.degree_distribution = scaler.min_max_range_normalization(array=distribution, min=0, max=max_degree)
    ...
    def rotate(self, img:np.ndarray, degree:int)->np.ndarray:
        ...
        trans_m = cv2.getRotationMatrix2D(center, degree, 1.0)   # 회전을 위한 변환 행렬 계산
    ...
    def _calculate_new_size(self, x:int, y:int, trans_m:np.ndarray)->Tuple[int, int]:
        """
...
class Shear:
    def __init__(
            self, p:float=1.0, min:float=-0.3, max:float=0.3, 
            how:int=0, keep_window:int=1, color:str='random', 
            distribution:Optional[np.ndarray]=None
        ):
        """
        이미지를 Shear 변환한다.
        ...
        if distribution is None:
            distribution = dist.linear(min=0, max=1, size=1000)
        self.dist = scaler.min_max_range_normalization(array=distribution, min=min, max=max)
    ...
    def shear_transformation(
            self, img:np.ndarray, 
            factor_x:float=0.0, factor_y:float=0.0, 
            adjust:bool=True
        )->np.ndarray:
        """
        ...
        x, y = self._adjustment_transformation_img(img, shear_m) if adjust else get_img_axis_size(img)
        # Shear 변환
    ...
    def _adjustment_transformation_img(self, img:np.ndarray, shear_m:np.ndarray)->Tuple[int, int]:
        """
    ...
    def _get_xy_factors(self)->Tuple[float, float]:
        ...
        """
        method = self.how_methods.get(self.how, self.how_methods[0])   # dictionary.get(key, default)
        return method()
...
class Translate:
    def __init__(
            self, p:float=1.0, min:float=0.05, max:float=0.2, 
            how=0, color='random', distribution:Optional[np.ndarray]=None
        ):
        """
        이미지를 평행 이동 변환 한다.
        >>> distribution은 임의의 분포인 배열이다 - 기본적으로 선형 분포를 갖는다.
        ...
        if distribution is None:
            distribution = dist.linear(min=0, max=1, size=1000)
        self.move_dist = scaler.min_max_range_normalization(array=distribution, min=min, max=max)
    ...
    def _random_translate_style(self)->Callable:
        ...
        """
        return self.color_padding if prob.binary_probability(p=0.5) else self.border_replicate
    ...
    def border_replicate(self, img:np.ndarray)->np.ndarray:
        ...
        replicated_img = cv2.copyMakeBorder(
            img, top=top, bottom=bottom, left=left, right=right, 
    ...
    def _crop_border_replicate_img(self, replicated_img:np.ndarray, x:int, y:int, move_x:int, move_y:int)->np.ndarray:
        """
    ...
    def _calculate_move_range(self, x:int, y:int)->Tuple[int, int]:
        ...
        # 방향 정의
        if prob.binary_probability(p=0.5): move_x = move_x * -1
        if prob.binary_probability(p=0.5): move_y = move_y * -1
        # 정수 변환
...
class Scaling:
    def __init__(
            self, p:float=1.0, min:float=0.7, max:float=1.3, 
            how:int=0, quality:str='low',
    ...
    def _select_how(self)->Tuple[float, float]:
        ...
        """
        how = np.random.choice(self.how_list, 1).item() if self.how_list is not None else self.how
...
class Resize:
    def __init__(
            self, size:int=256, quality:str='low', how:int=2, 
            how_list:Optional[List[int]]=None, color='random'
        ):
        """
    ...
    def _apply_padding(self, img:np.ndarray, padding_type:str)->np.ndarray:
        ...
        if self._check_pass_img(img=resized_img):
            ...
            if padding_type == "color":
                return base.padding(img=resized_img, pixel=RGBTuple.get(color=self.color))
...
class Flip:
    def __init__(
            self, p:float=1.0, how_list:Optional[List[str]]=None
        ):
        """
        이미지를 p의 확률로 flip 한다. 
...
class Crop:
    def __init__(
            self, p:float=1.0, min:float=0.1, max:float=0.3, color:str='random', 
            crop_style:int=0, crop_style_list:Optional[List[int]]=None,
            box_style:int=0, box_style_list:Optional[List[int]]=None,
        ...
        self.box_ins = _BoxStyle(
            dist=scaler.min_max_range_normalization(array=distribution, min=min, max=max),
    ...
    def apply_padding(self, img:np.ndarray, border_replicated:bool=True)->np.ndarray:
        """
        padding 적용
        """
    ...
    def apply_crop(self, img:np.ndarray, is_center:bool=True)->np.ndarray:
        ...
        # 시작점을 구한다.
        if is_center:
            start_x, start_y = self._center_start_point(x=x, y=y, new_x=crop_x, new_y=crop_y)
        else:
            start_x, start_y = self._random_start_point(x=x, y=y, new_x=crop_x, new_y=crop_y)
        return base.crop(img=img, x=start_x, y=start_y, x_size=crop_x, y_size=crop_y)


    def _center_start_point(self, x:int, y:int, new_x:int, new_y:int)->Tuple[int, int]:
        """
    ...
    def _random_start_point(self, x:int, y:int, new_x:int, new_y:int)->Tuple[int, int]:
        """
    ...
    def _check_error(self):
        ...
        """
        _CropError.check_style(style=self.crop_style, valid_styles=self.default_crop_style_list, key='crop')
        _CropError.check_style(style=self.box_style, valid_styles=self.default_box_style_list, key='box')
        _CropError.check_list(style_list=self.crop_style_list, valid_styles=self.default_crop_style_list, key='crop')
        _CropError.check_list(style_list=self.box_style_list, valid_styles=self.default_box_style_list, key='box')
...
class _CropError:
    ...
    """
    @classmethod
    def check_list(self, style_list:List[int], valid_styles:List[int], key:str='box'):
        """
        ...
        if not isinstance(style_list, list):
            raise TypeError(f"{key}의 style_list는 반드시 {valid_styles}에 포함된 값으로 구성된 list여야 합니다!")
        
        invalid_items = [i for i in style_list if i not in valid_styles]
        if invalid_items:
            raise ValueError(f"{key}의 style_list에는 유효하지 않은 값 {invalid_items}가 포함되어 있습니다. 반드시 {valid_styles}에 포함된 값으로 구성되어야 합니다!")
    ...
    def check_style(self, style:int, valid_styles:List[int], key='box'):
        ...
        if style not in extended_styles :
            raise ValueError(f"{key}의 style은 반드시 {extended_styles}에 포함된 정수 중 하나로 선택되어야 합니다!")