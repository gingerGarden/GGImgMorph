...
class Perspective:
    def __init__(
            self, p:float=1.0, min:float=-0.3, max:float=0.1, 
            how:int=0, color:str='random', 
    ...
    def _check_how(self):
        if self.how not in [0, 1, 2]:
            warnings.warn("how에는 0, 1, 2 3개의 정수 중 하나가 입력되어야 합니다! 기본값인 0으로 실행됩니다.")
            self.how = 0
    ...
    def transform(self, img:np.ndarray, extension:bool)->np.ndarray:
        ...
        # 원근 변환 적용
        return cv2.warpPerspective(img, trans_m, (x, y), borderValue=RGBTuple.get(color=self.color))
    ...
    def _destination_corners(self, x:int, y:int)->np.ndarray:
        ...
        dst_points = np.float32([
            [self._random_points(x, reverse=False), self._random_points(y, reverse=False)],
            [self._random_points(x, reverse=True), self._random_points(y, reverse=False)],
            [self._random_points(x, reverse=False), self._random_points(y, reverse=True)],
            [self._random_points(x, reverse=True), self._random_points(y, reverse=True)]
        ])
        return dst_points
    ...
    def _matrix_adjustment(self, m:np.ndarray, dst_corners:np.ndarray)->Tuple[int,int]:
        """
