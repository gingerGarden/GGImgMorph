...
class Circle:
    def __init__(
            ...
            how:int=0, how_list:Optional[List]=[1,2], color:str='random',
    ...
    def _make_radius(self, x:int, y:int, is_max:bool, make_half:bool)->int:
        ...
        if make_half: size = size/2
        return int(np.trunc(size * (1-prob.choose_one(self.dist))))
    

    def _make_center(self, x:int, y:int, radius:int, is_random:bool)->Tuple[int, int]:
        """
        중심 좌표 계산
    ...
    def _check_how(self):
        ...
        if self.how not in check_list:
            raise ValueError(f"how에 잘못된 값인 {self.how}가 입력되었습니다! {check_list}에 해당하는 값을 입력해야 합니다!")
    ...
    def _check_how_list(self):
        ...
        if len(self.how_list) == 0:
            raise ValueError("how_list에는 반드시 한 개 이상의 원소가 포함되어야 합니다!")
        if len(set(self.how_list) - set(self.how_keys)) > 0:
            raise ValueError(f"how_list에는 {self.how_keys}에 포함되는 값만 입력되어야 합니다!")
        

    def _check_adjustment(self):
        if self.adjustment == 1.0:
            warnings.warn("adjustment에 1.0이 입력되는 경우, 에러가 발생할 수 있습니다. adjustment를 0.99로 수정합니다.")
            self.adjustment = 0.99
        if self.adjustment <= 0.5:
            raise ValueError("ajustment에는 반드시 0.5를 초과하는 값이 입력되어야 하며, 0.5에 가까워질수록 frame이 이미지 안에 보이지 않을 수 있습니다!")



class Square:
    def __init__(
            self, p:float=1.0, min:float=0.05, max:float=0.3, 
            how:int=0, shuffle:int=0, color:str='random', 
            distribution:Optional[np.ndarray]=None
        ):
        """
        ...
        self.how = self._error_check_how(value=how, valid_map=self.size_methods.size_map, key='size_map')
        ...
        self.shuffle = self._error_check_how(value=shuffle, valid_map=self.shuffle_map, key='shuffle')
    ...
    def _get_size_dict_keys(self, size_dict:Dict[str, int], shuffle:bool)->List[str]:
        """
    ...
    def _fill_each_frame(
            ...
            size_dict:Dict[str, int], key:str, 
    ...
    def _size_tuple_to_size_dict(self, size_tuple:Tuple[int, int, int, int])->Dict[str, int]:
        """
        size_tuple(top, bottom, left, right)을 dictionary로 변환
        """
        return {'top':size_tuple[0], 'bottom':size_tuple[1], 'left':size_tuple[2], 'right':size_tuple[3]}
    

    def _error_check_how(self, value:int, valid_map:Dict[int, Callable], key:str='size_map'):
        """
...
class _SquareSizeMethod:
    ...
    def _how_1_frame_size(self, x:int, y:int)->Tuple[int, int, int, int]:
        ...
        """
        return (self._random_size(y), self._random_size(y), self._random_size(x), self._random_size(x))
