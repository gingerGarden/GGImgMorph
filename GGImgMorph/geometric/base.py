...
import warnings, cv2
import numpy as np
from GGUtils.img.img import get_img_axis_size, size_tuple_type_error
...
def padding(
    ...
    if x_pad_size == 0 and y_pad_size == 0:
        ...
        # 패딩
        return cv2.copyMakeBorder(img, top, bottom, left, right, cv2.BORDER_CONSTANT, value=pixel)
...
def _get_padding_size(x:int, y:int, size_tuple:Optional[Tuple[int, int]])->Tuple[int, int]:
    """
    ...
    if size_tuple is not None:
        size_tuple_type_error(size_tuple=size_tuple)   # size_tuple이 (int, int) 인지 확인.
...
def resize(img:np.ndarray, new_x:int, new_y:int, quality='low')->np.ndarray:
    ...
    # Resize
    return cv2.resize(img, (new_x, new_y), interpolation=interpolation_quality(quality, reduce))
