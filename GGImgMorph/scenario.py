...
import numpy as np
from GGStatify import distribution as dist
from GGStatify import probability as prob
    ...
    def _make_p_list(self, p_list:List[int])->List[int]:
        ...
        if p_list is not None:
            if len(self.fn_tuple) != p_list:
                raise ValueError("Augmentation method와 p_list의 길이가 다릅니다! 길이를 확인하십시오!")
...
# augment 예제
##########################################################################
from GGImgMorph.geometric import geometric as geo
from GGImgMorph.geometric import distortion as distor
from GGImgMorph.geometric import frame
...
_aug_last = OrderScenario(
    geo.Crop(p=0.4, color=_color)
)
sample_augment = OrderScenario(
    _aug_1, _aug_2, _aug_last
)
##########################################################################