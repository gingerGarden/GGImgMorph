...
import cv2
import numpy as np
from GGUtils.img.img import RGBTuple


class Circle:
    @staticmethod
    def make_mask(img:np.ndarray, center:Tuple[int, int], radius:int, none_value:int=300)->np.ndarray:
        """
    ...
    @staticmethod
    def add_mask(img:np.ndarray, mask:np.ndarray, none_value:int=300, color:str='random')->np.ndarray:
        """
        ...
        add_mask_img = np.where(add_mask_img >= none_value, RGBTuple.get(color), add_mask_img)
